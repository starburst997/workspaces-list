...
export class ClaudeCodeMonitor {
	 ...
	 */
	async getStatus(workspacePath: string): Promise<ClaudeCodeStatus | undefined> {
		try {
	 ...
	 */
	private async getWorkspaceConversations(workspacePath: string): Promise<ConversationMetadata[]> {
		...
		for (const cacheDir of ClaudeCodeMonitor.CACHE_DIRS) {
			try {
				const convos = await this.findConversationsInDir(cacheDir, workspacePath);
			...
			for (const entry of entries) {
				if (entry.isDirectory()) {
					...
					try {
						...
						if (this.matchesWorkspace(metadata.workingDirectory, workspacePath)) {
							conversations.push(metadata);
						}
					} catch {
						// Try looking for messages.jsonl or similar
						const messagesPath = path.join(cacheDir, entry.name, 'messages.jsonl');
						try {
							const metadata = await this.inferMetadataFromMessages(messagesPath);
							if (metadata && this.matchesWorkspace(metadata.workingDirectory, workspacePath)) {
								conversations.push(metadata);
							}
						} catch {
	 ...
	 */
	private async readConversationMetadata(metadataPath: string): Promise<ConversationMetadata> {
	 ...
	 */
	private async inferMetadataFromMessages(messagesPath: string): Promise<ConversationMetadata | null> {
		try {
	 ...
	 */
	private matchesWorkspace(conversationWorkspace: string, targetWorkspace: string): boolean {
		...
		return normalizedConv === normalizedTarget || normalizedConv.startsWith(normalizedTarget + path.sep);
	}
	 ...
	 */
	private async checkForWaitingInput(conversations: ConversationMetadata[]): Promise<boolean> {
	 ...
	 */
	private async checkForRunningConversation(conversations: ConversationMetadata[]): Promise<boolean> {
	 ...
	 */
	watchWorkspace(workspacePath: string, onChange: () => void): vscode.Disposable[] {
		const disposables: vscode.Disposable[] = [];

		// Watch all possible cache directories
		for (const cacheDir of ClaudeCodeMonitor.CACHE_DIRS) {
			try {
				const pattern = new vscode.RelativePattern(cacheDir, '**/*.{json,jsonl}');
